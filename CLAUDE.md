...
4. **Public header signatures are frozen by default.** If a bug
   ...
   `.hxx` file, stop and ask — name the exact signature change and why
   — rather than applying it. This is the one deliberate exception to
   "just proceed": header changes ripple to every other file that
   includes it, the same reasoning that kept `string.hxx` untouched.
...
`Status` is one of `pending`, `done`, or `generated`. `Order` is only
meaningful for `pending` rows; it's what PROCESS NEXT reads to pick the
next file. `generated` marks a file that's machine-produced by another
in-scope file (e.g. `src/dtreg.cxx`/`.hxx`, generated by
...
`generated` rows the same as `done` ones.
