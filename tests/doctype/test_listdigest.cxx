// Tests for doctype/listdigest.hxx / doctype/listdigest.cxx (class
// LISTDIGEST). Linked against the real implementation via
// TEST_ENGINE_DOCTYPE_SRCS in the top-level Makefile, not
// reimplemented here.
//
// LISTDIGEST's constructor forwards to MAILFOLDER(DbParent), so the
// same minimal TESTIDBOBJ shape used by tests/doctype/test_irlist.cxx
...
struct TempFile {
	...
	explicit TempFile(const std::string& Contents) {
		char tmpl[] = "/tmp/isearch2_test_listdigest_XXXXXX";
		int fd = mkstemp(tmpl);
...
TEST_CASE("LISTDIGEST::ParseRecords splits at a magic separator line", "[listdigest]") {
...
TEST_CASE("LISTDIGEST::ParseRecords does not add a bogus record when a magic separator is the first line", "[listdigest]") {
	// Confirms (empirically, not just by hand-trace) that the mid-loop
	// RecordEnd = SavePosition - 1 site cannot underflow here: Position
	// is always incremented by the *current* (matching) line's length,
	// at least magic_len+1 bytes, before SavePosition is ever captured
	// -- unlike doctype/irlist.cxx's more convoluted conditional
	// increment, where the equivalent site genuinely could underflow.
	std::string magic = "========================================\n";
...
TEST_CASE("LISTDIGEST::ParseRecords does not add a bogus record for an empty file", "[listdigest]") {
	...
	ld.ParseRecords(fileRecord);  // Must not crash.
