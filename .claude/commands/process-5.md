# Process up to 5 files from the Isearch2 cleanup queue

Batch version of `/process-next`: runs the same per-file pipeline up to
5 times in one invocation instead of stopping after one file. Intended
...
3. **Loop up to 5 times**, stopping early if `pending` rows run out.
   Each iteration:
   ...
   d. **If GENERAL blocked the file:** it already committed and pushed
      ...
      batch's 5-file loop budget (so a run of nothing but blocked files
      still terminates after 5, rather than spinning).
   e. **Otherwise:** update that file's row in
      ...
      summary.
4. **Stop** after 5 iterations (processed + blocked combined) or when
