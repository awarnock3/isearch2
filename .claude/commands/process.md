...
5. Run the full GENERAL pipeline on that one file exactly as defined in
   ...
   public header signatures unless you stop to ask, catalog bugs into
   `docs/BUG_CATALOG.md` with `BUGFIX #n` comments, modernize
   `NULL`→`nullptr` and `sprintf`→`snprintf`, add doc comments, write or
   update the Catch2 test under the mirrored `tests/` path, compile
   clean via `make tests`, run `make tests-asan`, refresh the
   `ISEARCH2-CLEANUP: processed` marker date).
6. Update or create that file's row in `docs/PROCESSING_STATUS.md`
   (status `done`, refreshed `Last Processed` date, updated
   `docs/BUG_CATALOG.md` link). If it wasn't in the table before, add
   it rather than requiring a prior `/analyze` run.
...
8. **Stop** after this one file.
9. Report back: what changed since the last pass (if any), bugs
   found/fixed this time, test results, compile/sanitizer status, and
   both commit hashes.
